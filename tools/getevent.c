...
static int print_event(int type, int code, int value, int print_flags)
{
    ...
    if (print_flags & PRINT_LABELS) {
        ...
        if (value == 0 && prev_value == 0xffffffff) return -1; 
        prev_value = value;
    }
    return 0;
}
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    int ret = print_event(event.type, event.code, event.value, print_flags);
                    if (ret == -1) {
                        printf("%s", newline);
                        return 0;
                    }
